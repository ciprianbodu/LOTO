...
def constraints(N, pick):
    ...
    for thr in (25, 30, 35, 40, 45):
        if thr < N:
            C[f"toate <= {thr}"] = (
                lambda d, t=thr: max(d) <= t,
                sum(1 for x in range(1, N + 1) if x <= thr),
            )
    for thr in (5, 10, 15):
        C[f"toate > {thr}"] = (
            lambda d, t=thr: min(d) > t,
            sum(1 for x in range(1, N + 1) if x > thr),
        )
...
print(
    f"{'joc':6} {'constrangere':16} {'obs%':>7} {'teoretic%':>10} {'|z|':>6} {'baza':>5} "
    f"{'P(6din baza)':>13}"
)
print("-" * 72)
rows = []
for g, (path, N, pick) in GAMES.items():
    ...
    for name, (fn, base) in constraints(N, pick).items():
        ...
        print(
            f"{g:6} {name:16} {obs / n * 100:6.2f}% {p_th * 100:9.2f}% {abs(z):6.2f} {base:5} "
            f"{p_th * 100:12.3f}%"
        )
print(
