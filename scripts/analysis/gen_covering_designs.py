...
def read_valid_design(path: Path, v: int, pick: int, g: int):
    ...
    return (
        blocks
        if coverage([[n - 1 for n in block] for block in blocks], v, g) == 100.0
        else None
    )
...
geos = [
    (v, pick, g)
    for pick in (5, 6)
    for g in ([3, 4] if pick == 5 else [3, 4, 5])
...
for i, (v, pick, g) in enumerate(geos, 1):
    ...
    t = time.time()
    gw, _ = wm._greedy_fallback(pool, pick, g, 0, None)
    t_greedy = time.time() - t
    n_greedy = len(gw)
    # ILP cu buget mare
    wm._ILP_COVER_CACHE.clear()
    t = time.time()
    cover = wm._ilp_cover_positions(v, pick, g, time_limit=BUDGET)
    t_ilp = time.time() - t
    ...
    assert coverage([[x - 1 for x in b] for b in best], v, g) >= 100.0, (
        f"C({v},{pick},{g}) NU e 100%"
    )
    ...
    if prev is not None:
        if len(prev) <= len(best):
            print(
                f"[{i:2}/{len(geos)}] C({v},{pick},{g}): PASTREZ fisierul existent "
                f"({len(prev)} <= {len(best)})",
                flush=True,
            )
            rows.append((v, pick, g, n_greedy, len(prev), "existent"))
            continue
    if path.exists() and prev is None:
        print(
            f"[{i:2}/{len(geos)}] C({v},{pick},{g}): inlocuiesc design invalid",
            flush=True,
        )
    path.write_text(
    ...
    print(
        f"[{i:2}/{len(geos)}] C({v},{pick},{g}): greedy={n_greedy:4}  ->  {len(best):4} ({src}) "
        f"{'*** -' + str(n_greedy - len(best)) if len(best) < n_greedy else ''}  [{t_ilp:.0f}s]",
        flush=True,
    )
