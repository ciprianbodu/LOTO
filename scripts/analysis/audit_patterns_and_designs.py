...
def holm(pvalues):
    ...
    for i, key in enumerate(ordered):
        previous = max(previous, min(1.0, (len(ordered) - i) * pvalues[key]))
        out[key] = previous
    return out
...
def metrics(hits, base_hits, universe, draw_n, pool):
    ...
    for target in (3, 4):
        ...
        result[f"p_{target}plus"] = float(
            binomtest(observed, len(h), theory, alternative="greater").pvalue
        )
        result[f"delta_{target}plus_pp"] = float(
            ((h >= target).mean() - (b >= target).mean()) * 100
        )
...
def audit_game(game, pool):
    ...
    # Contractul unic de validare (AGENTS.md §4.1) — inainte reimplementat
...
def target_matrix(blocks, v, condition, guarantee):
    ...
    return np.array(
        [(np.isin(targets, block).sum(axis=1) >= guarantee) for block in blocks]
    )
...
def audit_designs():
    ...
    for path in sorted((ROOT / "covering_designs").glob("[CL]_*.txt")):
        ...
        valid = bool(blocks) and all(
            len(b) == pick and len(set(b)) == pick and min(b) >= 1 and max(b) <= v
            for b in blocks
        )
        if not valid:
        ...
        if (
            not full
            or compute_coverage_pct(blocks, list(range(1, v + 1)), guarantee, condition)
            != 100.0
        ):
        ...
        per_block = sum(
            math.comb(pick, j) * math.comb(v - pick, condition - j)
    ...
    print(
        f"Designuri: {len(result)} validate, {sum(x['removable'] for x in result)} bilete eliminabile",
        flush=True,
    )
    return result
                ...
                for h in range(3, min(actual_n, pool) + 1):
                    for target in (3, 4):
                        ...
                        probabilities[target] += (
                            int(covered.sum())
                            * math.comb(universe - pool, actual_n - h)
                            / math.comb(universe, actual_n)
                        )
                row[f"draw_{actual_n}"] = {
                    f"p_ticket_{t}plus": p for t, p in probabilities.items()
                }
            rows.append(row)
        result[game] = rows
    return result
...
def prune_redundant_designs(audit):
    ...
    for row in audit:
        ...
        if (
            not (counts > 0).all()
            or compute_coverage_pct(
                reduced, list(range(1, v + 1)), guarantee, condition
            )
            != 100.0
        ):
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=ROOT / "bench_results/pattern_design_audit.json"
    )
    parser.add_argument(
        "--prune-redundant",
        action="store_true",
    ...
    result = {
        "protocol": "CPU; 50/20/30 cronologic; 8 ipoteze + frequency; fără promovare automată",
