...
def load(path, pick):
    ...
    for row in csv.DictReader(open(path, encoding="utf-8")):
        try:
            n = [int(row[f"n{i}"]) for i in range(1, pick + 1)]
        except Exception:
            continue
        if len(set(n)) == pick:
            out.append(sorted(n))
...
print(
    f"{'joc':6} {'trasatura':13} {'lag1':>7} {'lag2':>7} {'lag3':>7} {'p(perm,lag1)':>13}"
)
print("-" * 60)
worst = []
for g, (path, N, pick) in GAMES.items():
    ...
    feats = dict(FEATS)
    feats["cate <=mij"] = lambda d, h=N // 2: sum(1 for x in d if x <= h)
    for name, fn in feats.items():
        ...
        print(
            f"{g:6} {name:13} {acs[0]:+7.4f} {acs[1]:+7.4f} {acs[2]:+7.4f} {pval:13.4f}"
        )
worst.sort()
print(
...
print(
    f"Cate sub 0.05 fara corectie: {sum(1 for w in worst if w[0] < 0.05)} (asteptat din hazard: {0.05 * len(worst):.1f})"
)
