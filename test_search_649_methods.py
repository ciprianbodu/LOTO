...
def _result(method: str, rate: float = 0.15, lift: float = 0.25) -> sm.SearchResult:
    return sm.SearchResult(
        method=method, rate_4plus_k16=rate, lift_vs_baseline=lift, failed=False
    )
...
def test_top649_alias_matches_between_generate_and_patch():
    ...
    top = [
        _result("649_blend_katz_gap_x"),
        _result("autocorr", rate=0.14),
        _result("dmd", rate=0.13),
    ]
...
def test_patch_best_methods_writes_single_member_ensemble_with_resolvable_alias(
        ...
        json.dumps(
            {
                "games": {
                    "loto_6_49": {
                        "auto_pilot_per_pool": {
                            "k11": {"scorer": "existing", "avg_hits": 1.0}
                        }
                    }
                },
            }
    ...
    top = [
        _result("649_blend_new_candidate", rate=0.20, lift=0.30),
        _result("autocorr", rate=0.18),
        _result("dmd", rate=0.17),
    ]
...
def test_generate_methods_top649_uses_same_alias_scheme(tmp_path, monkeypatch):
    ...
    text = (tmp_path / "loto_enterprise" / "benchmark" / "methods_top649.py").read_text(
        encoding="utf-8"
    )
    assert (
        '"top649_01_649_blend_new_candidate": METHODS["649_blend_new_candidate"]'
        in text
    )
