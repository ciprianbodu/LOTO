...
print(
    f"  Așteptat uniform: {exp:.1f} apariții/număr | χ²={chi2:.1f} (df={dfree}) p={p:.3f}"
)
if p < 0.05:
...
print(
    "  ⚠️  NOTĂ: 'overdue' NU crește șansa — extragerile sunt independente (gambler's fallacy)"
)
...
print(
    f"  Observat: medie={sums.mean():.1f} std={sums.std():.1f} | Teoretic medie={theo_mean:.1f}"
)
print(
    f"  Interval p10-p90: {np.percentile(sums, 10):.0f} – {np.percentile(sums, 90):.0f}"
)
t, pt = stats.ttest_1samp(sums, theo_mean)
print(
    f"  t-test vs medie teoretică: p={pt:.3f}"
    + ("  ⚠️ deviază" if pt < 0.05 else "  ✅ normal")
)
...
for _ in range(20000):
    ...
    if any(s[i + 1] - s[i] == 1 for i in range(len(s) - 1)):
        sim += 1
print(
    f"  Observat: {has_consec}/{n_rec} extrageri ({100 * has_consec / n_rec:.1f}%) cu ≥1 pereche consecutivă"
)
...
print(
    f"  Așteptat/pereche ≈ {exp_pair:.2f} | Top: "
...
print(
    f"  z-score carryover vs hazard: {z:+.2f}"
...
if signals:
    print(
        "⚠️  Semnale peste prag (necesită Monte-Carlo de confirmare, pot fi fals-pozitive):"
    )
    for s in signals:
        print(f"     • {s}")
    print(
        "   La 9 teste, ~0.4 fals-pozitive sunt așteptate doar din hazard (Bonferroni)."
    )
else:
