...
def test_gate_uses_hypergeometric_reference_not_noisy_random_row(monkeypatch):
    ...
    cfg2 = decision.decide_optimal_config_for_pool(
        pd.DataFrame(rows2), "synthetic_game", 10, 6
    )
...
def test_method_with_missing_window_is_excluded_and_reported(monkeypatch):
    ...
    assert cfg["incomplete_methods"] == [
        {"method": "m_incomplete", "missing_windows": [60]}
    ]
...
def test_tiebreak_dependent_method_is_excluded_only_when_column_exists(monkeypatch):
    ...
    for pct, n in ((10, 100), (30, 300), (60, 600), (100, 1000)):
        ...
        rows.append(
            _row("loto_6_49", "m_tiebreak", pct, n, base + 0.10, tiebreak_k10=0.9)
        )
    cfg = decision.decide_optimal_config_for_pool(
        pd.DataFrame(rows), "loto_6_49", 10, 6
    )
    ...
    assert cfg["scorer"] == "m_alpha"
    assert cfg["tiebreak_dependent"] == [
        {"method": "m_tiebreak", "tiebreak_fraction": 0.9}
    ]
...
def test_partially_evaluated_window_cannot_qualify_or_win_fallback(monkeypatch):
    monkeypatch.setattr(decision, "BENCH_HIT_TARGET", 3)
    for complete_rate in (0.15, 0.01):
        rows = []
        for pct, n in ((10, 100), (30, 300), (60, 600), (100, 1000)):
            rows.append(_row("loto_6_49", "m_alpha", pct, n, complete_rate))
            rows.append(_row("loto_6_49", "m_incomplete", pct, n, 0.9, n_eval=n - 1))
        cfg = decision.decide_optimal_config_for_pool(
            pd.DataFrame(rows), "loto_6_49", 10, 6
        )
        assert cfg["scorer"] == "m_alpha"
        assert cfg["incomplete_methods"][0]["reason"] == "unevaluated_draws"
...
def test_exact_ties_are_broken_by_name_regardless_of_row_order(monkeypatch):
    ...
    b = decision.decide_optimal_config_for_pool(
        pd.DataFrame(rows[::-1]), "loto_6_49", 10, 6
    )
...
def test_fold_reports_tiebreak_fraction_per_pool(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        (
            lambda _d, max_num: {
                n: (1.0 if n % 2 == 0 else 0.0) for n in range(1, max_num + 1)
            },
            "test",
            False,
            "sintetic",
    ...
    train = np.array(
        [sorted(rng.choice(np.arange(1, 50), 6, replace=False)) for _ in range(20)]
    )
    test = np.array(
        [sorted(rng.choice(np.arange(1, 50), 6, replace=False)) for _ in range(6)]
    )
