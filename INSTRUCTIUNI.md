...
benchmark de ~130 metode CPU (statistice, ML sklearn, graf/network, geometrice)
și validare walk-forward onestă.
...
1. Verifică mediul (Python **3.14+**, venv `D:\_BUILD\_LOTO\.venv`, importuri CPU).
...
```
Încarci CSV-uri  →  [🟠 RE-BENCH]  →  (auto) [⚡ Auto-Pilot]  →  Rezultate
...
2. **Setări** — Dimensiune Pool (6–16), Garanție, Buget walk-forward (implicit **90 min**), etc.
3. **🟠 RE-BENCH** — testează toate metodele CPU pe istoric, alege câștigătorul per joc (scrie `best_methods.json`). Durează **ore** (lasă peste noapte). Dacă bifa „Auto-Pilot automat după Re-Bench" e pornită → **pornește singur Auto-Pilot** la final.
...
- **🏆 Metodă câștigătoare** — ce model a generat pool-ul (ex. `seasonal_naive`), cu descriere + familie.
- **⚖️ Ensemble** — combină top-3 metode calificate (variance-reduction).
...
- **📊 Walk-forward** — validarea onestă: pe ultimele **30%** extrageri. **Pool 1 + OMNIUS 1** mereu; cu **Inversare automată** ON → și **Pool 2 + OMNIUS 2** (Faza 1 → excludere → Faza 2). Rulează **Joker → 5/40 → 6/49**.
...
| 50–100+ extrageri noi / câteva luni | **Re-Bench** o dată, să re-verifici metoda câștigătoare |
...
- **Buget walk-forward** — plafon de siguranță (implicit **90 min**), nu timp real. Cu WF paralel (~80% CPU), validarea la 30% depth durează de obicei **câteva minute**; bugetul mare e doar ca să nu oprească prematur la anulare automată.
...
| Fișier | Ce e |
|---|---|
| `_ISTORIC/` | datele tale (CSV-uri cu extrageri) — versionate pe GitHub |
...
- **`ACTUALIZARI.bat`** — actualizează pachetele venv-ului (rar, după upgrade-uri). Instalează exclusiv stack CPU (`requirements_base.txt`).
