@echo off
REM ACTUALIZARI.bat - mediu venv + CSV + git sync.
REM ASCII only. No delayed expansion at file scope.
REM Delayed expansion is ONLY inside :CleanGhosts.
REM Do not caret-escape parentheses in echo/REM: delayed expansion
REM then leaves an open parenthesis and the next word runs as a command.
setlocal
cd /d "%~dp0"
...
REM Asigura directorul de build, in afara OneDrive.
if not exist "D:\_BUILD\_LOTO" mkdir "D:\_BUILD\_LOTO"

REM Curatare resturi din versiuni vechi: backup-uri venv cu sufix,
REM plus venv-ul vechi relativ .venv, mutat la D:\_BUILD\_LOTO\.venv.
...
if not exist "%VENV_PY%" (
    ...
    if errorlevel 1 (
        echo [EROARE] Creare venv esuata. Verifica: py -3.14 --version
        pause
        exit /b 1
    )
...
REM Daca interpretorul 3.14 lipseste de pe sistem, il INSTALAM automat.
...
REM Sterg venv-ul vechi DIRECT, fara backup.
...
if not errorlevel 1 (
    echo [WARN] update_csv.py a intampinat erori, posibil offline - continui cu istoricul existent.
)
...
if errorlevel 1 (
    echo   [WARN] Purge cache WF esuat - continui, posibil import/disk.
)
echo.
...
echo ------------------------------------------------------------
echo  Python: ACTUALIZARI.bat instaleaza/migreaza automat la 3.14
echo    via winget sau installer python.org. Daca instalarea auto
echo    esueaza, ia-l manual de la https://www.python.org/downloads/
echo    bifeaza Add to PATH, si reruleaza.
echo.
...
:check_integrity
REM Integritate venv: Python ruleaza? dependinte coerente via pip check?
REM librariile critice se importa - prinde coruptie binara .dll/.pyd OneDrive.
...
if "%SYS_VER%"=="" (
    ...
    echo           instaleaza manual de la https://www.python.org/downloads/ si bifeaza Add to PATH.
) else (
