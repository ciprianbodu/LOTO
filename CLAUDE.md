...
- **Wheeling** (`wheeling_methods.py` din rădăcină; env `LOTO_WHEEL_METHOD`). **Sursa unică a listei de valori = `wheeling_methods.py` (docstring-ul de modul + dict-ul `WHEEL_METHODS`)**: `ilp|annealing|genetic|lajolla|union34`, iar `greedy` **și orice valoare necunoscută** cad pe `_greedy_fallback`. ⚠️ Comentariul din `loto_engine.py` (locul care CITEȘTE env-ul) enumeră doar 4 valori (fără `greedy`/`union34`) — e o listă incompletă, nu o restricție; nu-l folosi ca referință.
  ...
  - ⚠️ **Filtrul anti-anomalie e DEZACTIVAT** în pipeline (`loto_engine.py`, „Fără filtru anti-anomalie — păstrăm toate variantele"; `audit.filters_disabled`). Helperul `wheeling_methods.filter_preserving_coverage` (elimină bilete DOAR dacă rămân redundante) **există dar nu are niciun call-site** — de folosit OBLIGATORIU dacă vreodată se reactivează un filtru post-wheel, ca să nu spargă garanția. `compute_coverage_pct` = API pt revalidarea acoperirii după orice filtrare.
    Reziduu simetric în UI: cheia `audit["anomaly_filter"]` **nu mai are niciun PRODUCER** (engine-ul n-o mai scrie), dar are doi CONSUMATORI morți în `app_nicegui.py` (randarea „🔍 Filtru anti-anomalie…" și explicația discrepanței de variante) — ramuri care nu se mai execută niciodată; de curățat odată cu reziduul OMNIUS.
- **Teste** (rădăcina repo, `pytest test_*.py`) — 8 fișiere: `test_adaptive_feedback.py`, `test_decision.py`, `test_engine_opt.py`, `test_integration.py`, `test_method_selector.py`, `test_queue_payload.py`, `test_reset_jobs.py`, `test_tfm_hits.py`.
  ⚠️ **Wheeling-ul NU are NICIUN test** (`grep -l wheel test_*.py` → zero hituri): `wheel_ilp`, `wheel_union34`, `generate_wheel` și `filter_preserving_coverage` sunt netestate. După orice modificare acolo, revalidează manual acoperirea cu `compute_coverage_pct`.
...
- **Walk-forward** (`loto_enterprise/core/walk_forward_adapter.py`: `run_honest_walk_forward` → `loto_enterprise/core/backtesting.run_retroactive_backtest`): validare Faza 1 (pool normal) + **Faza 2 (auto_invert)** când e ON; `hits_union` (pool) per extragere — **fără `omnius_hits`**; `progress_cb` + bară determinată; **depth = 30%** (`WF_DEPTH_PERCENT` în app_nicegui.py). Cache Pool 2: sufix `_inv.pkl`.
  ...
  - `CACHE_VERSION` PROPRIU în `walk_forward_adapter.py` (acum **`v13`**, separat de cel din `bench_cache.py`, acum `v10`) — bump la orice schimbare a STRUCTURII flat-ului SAU a pool-ului GENERAT (ex. decorelarea membrilor de ensemble schimbă pool-ul, deci cache-urile vechi validau un pool care nu se mai generează). ⚠️ Bump-ul **doar schimbă numele fișierului** — pickle-urile vechi rămân pe disc la nesfârșit (și se sincronizează prin OneDrive); curăță-le manual periodic sau cu `clear_walk_forward_cache()` (șterge TOATE versiunile, inclusiv cea curentă → rulează-l ÎNAINTE de primul WF nou).
  ...
  - **Buget de timp**: `wf_budget_min` (implicit **90 min**, persistat în UI) + `should_cancel`. La depășire → oprire PARȚIALĂ (extrageri recente) și pipeline continuă (mail/shutdown).
    ⚠️ Default-ul e exprimat în TREI locuri din `app_nicegui.py`, iar unul e DIVERGENT: `DEFAULTS["wf_budget_min"] = 90` și `WF_TOTAL_BUDGET_S = 90 * 60` sunt de acord, dar calculul bugetului folosește `float(SETTINGS.get("wf_budget_min") or 15)` — rest din bugetul vechi de 15 min. `or` se declanșează pe orice valoare FALSY, nu doar absentă (câmpul UI golit dă `None`) → bugetul devine tăcut 15 min. Când se atinge codul, aliniază fallback-ul la `DEFAULTS["wf_budget_min"]`.
...
8. **Top-N după scor** → în cod NOU, mereu `core.ranking.rank_by_score` (vezi „Tie-break canonic"); nu scrie sortare proprie.
   ...
   Sunt datorie tehnică CUNOSCUTĂ, nu excepții legitime. Migrarea lor la `rank_by_score` schimbă output-ul pe egalități → încalcă regula 1 (bit-identitate) și cere bump de `CACHE_VERSION` în `walk_forward_adapter.py`; de făcut într-un commit dedicat, nu în treacăt.
