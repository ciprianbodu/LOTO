...
| `loto_enterprise/core/walk_forward_adapter.py` | walk-forward pt UI (`run_honest_walk_forward`); `CACHE_VERSION` PROPRIU (`v22`), separat de cel din `bench_cache.py`; `CACHE_DIR = Path("bench_results")` (relativ → ÎN repo/OneDrive) |
...
- Re-Bench: walk-forward pe folduri → `bench_results/folds.csv` (OVERWRITE) → `decision.py` → `best_methods.json` (winner + sim_depth per joc/pool). **Țintă hituri = `BENCH_HIT_TARGET` (env `LOTO_BENCH_TARGET`, default 3+)** pentru 6/49, 5/40 și Joker Urna 1; clasamentul arată și 3+ și 4+. **Joker Urna 2 este benchmark-uit separat top-1 (1/1)**, cu baseline exact 1/20 = 5%.
  ⚠️ `LOTO_BENCH_TARGET` acceptă REALMENTE doar **3** sau **4**: pentru jocurile de pool `runner.py` emite coloanele `rate_3plus_*` / `rate_4plus_*`; pentru Urna 2 emite `rate_1plus_k1`, independent de setare. Orice altă valoare (ex. 5) e **CLAMPATĂ la 3** de `hit_target.clamp_bench_hit_target`, cu WARNING explicit în log (verificat: `LOTO_BENCH_TARGET=5` → `BENCH_HIT_TARGET = 3`). Fallback-ul de COLOANĂ rămâne posibil separat (folds.csv fără coloanele per-pool ale țintei) și e semnalizat prin `rate_col_mismatch`.
- **Decizie robustă (Wilson) + ensemble**: decision.py filtrează întâi pe **gate-ul de consistență** — o metodă e „calificată" doar dacă bate baseline-ul `random` în ≥60% din ferestre (`CONSISTENCY_THRESHOLD = 0.60`) pe **ACEAȘI rată T+ și același pool kN** folosite la alegerea câștigătorului. Gate-ul vechi folosea media `kN`: mai multe extrageri de 2 hituri puteau califica o metodă care pierdea față de random exact la 3+/4+. Coloanele fără sufix (`rate_Tplus`) sunt valide doar la pool-ul de bază (`K=draw_n`), nu ca fallback pentru alt kN; metodele fără rata comună sunt sărite. Apoi sortează calificatele după limita inferioară Wilson a ratei T+ și scrie `ensemble` (top-`ENSEMBLE_MAX_METHODS`=3 calificate, pondere ∝ Wilson) în `auto_pilot_per_pool[kN]`. Dacă NICIO metodă nu trece gate-ul → ramură de fallback cu `low_confidence=True` (alegerea e conservatoare, nu „cea mai consistentă").
 ...
 - **Denominatori folds (bench `CACHE_VERSION` v13)**: `_evaluate_fold` împarte TOT (hits_per_pool[_bl], avg_hits_topk, ratele T+) la `n_eval` (extrageri efectiv evaluate — exclude blocurile în care scorerul a întors scoruri goale/plate/ne-numerice/ne-finite); înainte hits împărțeau la `n_test`, iar ratele la `n_eval` → axele erau inconsistente. Câmp nou `FoldResult.n_eval` (coloană în folds.csv). Din `v16`, regula comună `core.score_validation.has_usable_score_variance` este folosită de benchmark, selector și engine, ca o metodă respinsă în producție să nu poată câștiga benchmarkul prin tie-break pe scor plat.
  - **Ordinea din clasamentul UI = ordinea deciziei** (aliniate). UI-ul IMPORTĂ
   ...
   ACELEAȘI chei: `(Wilson_lb, lift mediu ponderat al ratei T+ vs random, consistență n_beat/n_total pe rata T+)`.
...
- **Wheeling** (`wheeling_methods.py` din rădăcină; env `LOTO_WHEEL_METHOD`). **Sursa unică a listei de valori = `wheeling_methods.py` (docstring-ul de modul + dict-ul `WHEEL_METHODS`)**: `ilp|annealing|genetic|lajolla|union34`, iar `greedy` **și orice valoare necunoscută** cad pe `_greedy_fallback`.
  - `max_variants == 0` (fără cap de bilete) → implicit **`lajolla`**: design precalculat din `covering_designs/`, acum pentru **toate** geometriile accesibile din UI (pool 6-16 × pick 5/6 × garanție 3..pick-1). Ce nu are fișier cade pe ILP, iar ILP pe greedy. Câștigul măsurat 54→41 bilete e **doar** 6/49 pool 12 / g4; 5/40+Joker pool 12 / g4: 123→113. Lanț monoton: niciodată mai multe bilete decât înainte, aceeași garanție 100%.
  ...
  - **`wheel_union34`** este un alias compatibil pentru un singur cover `guarantee=4`: matematic, orice cover complet 4-din-4 acoperă deja și fiecare 3-din-3 (o triplă se extinde la o cvadruplă). Vechea uniune 3∪4 adăuga bilete fără să adauge garanție. Pentru `guarantee>4`, deleagă cererea reală către La Jolla/ILP/greedy, fără să pretindă acoperire 5+ dintr-un cover 4.
...
- **Cache fold** (`D:\_BUILD\_LOTO\.bench_cache`, în AFARA OneDrive; env `LOTO_BENCH_CACHE_DIR`): keyed pe `CACHE_VERSION`+csv_hash+game+method+pct+is_random. **Bump `CACHE_VERSION` din `bench_cache.py` (acum **`v16`**) când se schimbă output-ul unei metode, structura FoldResult sau regulile de validare ale extragerilor/scorurilor** (altfel servește stale). Date noi = re-bench COMPLET (hash pe tot istoricul).
...
- **Git: totul pe `main`.** Eu (asistent) push pe main; `START_8000.bat` face `git pull` automat la pornire. `best_methods.json`/`pool_history.json`/`raport_complet.txt` sunt gitignore. **`_ISTORIC/` E VERSIONAT** (sursă de adevăr pt bench/analiză); `START_8000.bat`+`ACTUALIZARI.bat` își copiază întâi launcherul + `loto_git_sync.bat` în `%TEMP%`, fac sync din copiile IMUTABILE, apoi transferă controlul la launcherul nou din repo (`--post-sync`). Ocolirea bootstrap-ului reintroduce bug-ul CMD de offset: `git reset` înlocuiește `.bat`-ul care rulează, iar revenirea din `CALL` execută un fragment de linie din fișierul nou. La sync forțat, helperul păstrează atât modificările tracked în stash, cât și commit-urile locale ahead într-un branch `backup/auto-sync-*`. Checkout **main**, commit `_ISTORIC`, **`git push origin main`**, mesaje în consolă — nu `HEAD` pe altă ramură, nu `>nul`.
...
- **Garanție UI 3..6**: `6` este expusă pentru sistemul complet 6/49; workerul o plafonează la `draw_n`, deci 5/40/Joker folosesc maximum 5. UI afișează garanția efectivă din rezultat.
...
- **Walk-forward** (`loto_enterprise/core/walk_forward_adapter.py`: `run_honest_walk_forward` → `loto_enterprise/core/backtesting.run_retroactive_backtest`): validare **doar Pool 1** (honest WF); Pool 2 când auto-invert e ON = istoric **retrospectiv** (pool+wheel curent pe aceleași extrageri, `_ensure_retrospective_pool2_flat`) — fără dublarea timpului WF. `hits_union` = **pool ∩ extragere** (nu uniunea biletelor). Cu acoperire 100% și `guarantee ≥ 3`, 3+ în pool ⇔ 3+ pe un bilet; 5+/6 rămân un **plafon** (WF intern e g=4, nu g=5). Sub acoperire <100% `hits_union` **supra-numără** vs bilete (oglinda bug-ului vechi). WF **persistă** acum acoperirea per pas (`wheel_coverage`, vezi mai jos) — `max_variants=0` țintește 100%, dar greedy-ul cu timeout de 1000 de iterații poate rămâne sub, iar diferența nu mai e tăcută. `hits` = max pe UN bilet (ce plătește). `progress_cb` + bară determinată; **depth = 30%** (`WF_DEPTH_PERCENT` în app_nicegui.py). Backtesting acceptă încă `auto_invert` pe pas (2× pipeline) dacă e cerut explicit; UI-ul nu-l folosește pe path-ul WF.
  ...
  - `CACHE_VERSION` PROPRIU în `walk_forward_adapter.py` (acum **`v22`**, separat de cel din `bench_cache.py`, acum **`v16`**) — bump la orice schimbare a STRUCTURII flat-ului, a pool-ului GENERAT, SAU a wheel-ului (algoritm + garanția internă a WF). Semnătura `_decision_sig` include scorer/ensemble/target **plus** `_wheel_sig` (metodă, garanție și hash-ul fișierului covering-design pentru La Jolla/union34); pentru Joker include și decizia top-1 a Urnei 2, deoarece ea schimbă numărul atașat fiecărui bilet. ⚠️ Bump-ul **doar schimbă numele fișierului** — pickle-urile vechi rămân pe disc; curăță-le cu `purge_stale_wf_cache(dry_run=False)` (doar versiuni ≠ curentă) sau `clear_walk_forward_cache()` (TOATE, inclusiv curentă → rulează-l ÎNAINTE de primul WF nou).
