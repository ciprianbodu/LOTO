...
- **Auto-invert / manual_blacklist — ELIMINATE** (2026-08-09): cerere user „niciun manual blacklist nici la pool1 nici la pool2". Worker-ul rulează o singură trecere; UI arată un singur pool; toggle-ul „Inversare automată" a dispărut. Parametrul `auto_invert` din config/WF e ignorat (compat payload).
...
- **Walk-forward** (`loto_enterprise/core/walk_forward_adapter.py`: `run_honest_walk_forward` → `loto_enterprise/core/backtesting.run_retroactive_backtest`): validare pe un singur pool (`auto_invert` / Faza 2 IGNORATE); `hits_union` (pool) per extragere; `progress_cb` + bară determinată; **depth = 30%** (`WF_DEPTH_PERCENT` în app_nicegui.py).
...
8. **Top-N după scor** → în cod NOU, mereu `core.ranking.rank_by_score` (vezi „Tie-break canonic"); nu scrie sortare proprie.
   Path-ul principal e migrat: trunchierea pool-ului și Urna 2 Joker folosesc `rank_by_score`. Path-ul `manual_blacklist` / auto-invert a fost eliminat. Mai rămâne un `sorted(..., reverse=True)` activ în `_get_timesfm_pool` (completare defensivă din blacklist când pool-ul e incomplet) și unul în `generate_combinatorial_wheel` (ordonare pool pt wheel, nu top-N subset). Migrarea lor pe egalități schimbă output-ul → încalcă regula 1; de făcut într-un commit dedicat, nu în treacăt.
