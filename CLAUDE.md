...
- **Wheeling** (`wheeling_methods.py` din rădăcină; env `LOTO_WHEEL_METHOD`). **Sursa unică a listei de valori = `wheeling_methods.py` (docstring-ul de modul + dict-ul `WHEEL_METHODS`)**: `ilp|annealing|genetic|lajolla|union34`, iar `greedy` **și orice valoare necunoscută** cad pe `_greedy_fallback`.
  - `max_variants == 0` (fără cap de bilete) → implicit **`lajolla`**: design cunoscut-optim din `covering_designs/` **doar pentru v=12** (`C_12_6_4.txt`, `C_12_5_4.txt`). Orice pool ≠ 12 cade pe ILP, iar ILP pe greedy. Câștigul măsurat 54→41 bilete e **doar** 6/49 pool 12 / g4; 5/40+Joker pool 12 / g4: 123→113. Lanț monoton: niciodată mai multe bilete decât înainte, aceeași garanție 100%.
  - `max_variants > 0` (buget fix) → **greedy**, apoi `ensure_pool_numbers_on_tickets`: un număr lipsă din pool înlocuiește pe un bilet un număr care apare deja pe altul (duplicat). Plafonul de bilete e respectat; acoperirea se recalculează. Path-ul default `max_variants=0` e neschimbat (regula 1). Un cap de 1 bilet nu poate acoperi un pool mai mare decât `pick` — numerele unice de pe bilet rămân.
