...
def main() -> int:
    ...
    if "rate_4plus" not in df.columns:
        print(
            "folds.csv e vechi (fără coloana rate_4plus) — rulează un Re-Bench cu codul nou."
        )
        return 1
    ...
    for game, gdf in df.groupby("game"):
        ...
        by_pct = (
            gdf.groupby("percentile")["rate_4plus"].mean().sort_values(ascending=False)
        )
        ...
        print(
            f"  ➜ CEL MAI BUN procent (mediu): {best_pct}%  (4+: {by_pct.iloc[0] * 100:.2f}%)"
        )
        # Cea mai bună combinație individuală metodă × percentilă
        best = gdf.loc[gdf["rate_4plus"].idxmax()]
        fam = (
            f" [{best['family']}]"
            if "family" in gdf.columns and pd.notna(best.get("family"))
            else ""
        )
        print(
            f"  ➜ MAXIM absolut: {best['method']}{fam} @ {int(best['percentile'])}%  "
            f"→ 4+: {best['rate_4plus'] * 100:.2f}%"
        )
        print(
            "  (ATENTIE: maximul de mai sus e cea mai buna din multe celule "
