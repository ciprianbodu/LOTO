...
def load_draws(game: GameDef) -> np.ndarray:
    ...
    game.history_cutoffs = training_cutoffs(df.loc[valid_mask].reset_index(drop=True))
    return draws
...
def _evaluate_fold(
    ...
    try:
        ...
        while pos < n_test:
            ...
            prefix_end = len(train_draws) + pos
            ...
            ranked = _top_k(scores, max(pool_sizes) + 1)
            top_sets = {k: set(ranked[:k]) for k in pool_sizes}
            ...
            ranked_bl = _top_k(filtered_scores, max(pool_sizes))
            top_sets_bl = {k: set(ranked_bl[:k]) for k in pool_sizes}
    ...
    for game in games:
        ...
        if use_cache and _cache_ok:
            try:
                ...
                # Dates change what can be observed before each target, even
                # when all number columns are unchanged.
                csv_hash_game = _hl.md5(
                    (csv_hash_game + repr(game.history_cutoffs)).encode()
                ).hexdigest()[:16]
                ...
                for is_random in (False, True) if shuffled_control else (False,):
                    ...
                    if cached is not None:
                        ...
                        args = (
                            ...
                            replace(game, history_cutoffs=()) if is_random else game,
                            block_size,
                            pct,
                            is_random,
                            csv_hash_game,
