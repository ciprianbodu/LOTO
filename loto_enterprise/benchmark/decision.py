...
def decide_optimal_config_for_pool(
    ...
    def _complete_windows(m: str, real_m: pd.DataFrame, gate_col: str) -> bool:
        if "n_eval" in real_m.columns and "n_test" in real_m.columns:
            evaluated = pd.to_numeric(real_m["n_eval"], errors="coerce")
            requested = pd.to_numeric(real_m["n_test"], errors="coerce")
            partial = evaluated.notna() & requested.notna() & (evaluated < requested)
            if partial.any():
                incomplete_methods.append(
                    {
                        "method": m,
                        "missing_windows": sorted(
                            map(int, real_m.loc[partial, "percentile"])
                        ),
                        "reason": "unevaluated_draws",
                    }
                )
                return False
