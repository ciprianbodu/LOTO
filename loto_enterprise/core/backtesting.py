...
from loto_enterprise.core.history import (
...
_WF_STEP_TIMEOUT_S = float(
    __import__("os").environ.get("LOTO_WF_STEP_TIMEOUT_S", "900")
)
...
_WF_PROBE_BUDGET_S = float(
    __import__("os").environ.get("LOTO_WF_PROBE_BUDGET_S", "2.0")
)
_WF_SHARED: dict = {}
...
def _retroactive_step_stateless(
    ...
    prefix_end = cutoffs[sim_idx]
    historical_df = df.iloc[:prefix_end].copy()
...
class BacktestResult:
    ...
    draw_index: int = 0
    hits_union: int = (
        0  # Câte numere s-au potrivit în întregul pool (union) la această extragere
    )
...
class LotoBacktester:
    ...
    def _load_data(self) -> None:
        ...
        if self.df is None:
            ...
            logger.info(
                f"[BACKTEST] Date încărcate: {len(self.df)} rânduri din {self.csv_path}"
            )
        else:
            logger.info(
                f"[BACKTEST] Folosesc DataFrame direct ({len(self.df)} rânduri)"
            )

        self.df = chronological_history(self.df)
        ...
        try:
            ...
            raise ValueError(
                f"Extrageri invalide pentru {self.game_type}: {exc}"
            ) from exc
        ...
        self.df.attrs["training_cutoffs"] = training_cutoffs(self.df)
        self.draws = [list(row) for row in matrix]
        date_col = next(
            (
                c
                for c in ["date", "data", "draw_date", "extragere", "Data"]
                if c in self.df.columns
            ),
            None,
        )
        self.dates = (
            [str(v) if pd.notna(v) else None for v in self.df[date_col]]
            if date_col
    ...
    def get_last_percentile_draws(
        self, percentile: float = 20.0
        ...
        logger.info(
            f"[BACKTEST] Perioada evaluare: ultimele {n_eval} extrageri din {n_draws} ({percentile}%)"
        )
        return result

    def evaluate_variant(
        self, variant: list[int], target_draws: list | None = None
    ) -> list[BacktestResult]:
        """
    ...
    def evaluate_variants(
        self, variants: list[list[int]], percentile: float = 20.0
    ) -> BacktestSummary:
        """
        ...
        logger.info(
            f"[BACKTEST] Vectorized eval: {V}×{D}={V * D} pairs în {elapsed_ms:.0f}ms"
        )
        ...
        for r in top_performing:
            r.hit_numbers = (
                set(self._scored_variant_numbers(r.variant)) & r.draw_numbers
            )
        ...
        summary = BacktestSummary(
            ...
            avg_union_hits=float(np.mean(union_hits_array))
            if len(union_hits_array) > 0
            else 0.0,
        ...
        logger.info(
            f"[BACKTEST] Evaluare completă: {summary.variants_tested} variante × {summary.total_draws_evaluated} extrageri"
        )
        return summary
    ...
    def print_summary(self, summary: BacktestSummary) -> None:
        ...
        for hits in sorted(summary.distribution.keys(), reverse=True):
            ...
            pct = (
                count / (summary.variants_tested * summary.total_draws_evaluated) * 100
            )
            bar = "#" * int(pct / 2)
        ...
        if summary.top_performing_draws:
            ...
            for i, r in enumerate(summary.top_performing_draws[:5], 1):
                ...
                print(
                    f"  {i}. {date_str}: {r.hits} numere ({', '.join(map(str, sorted(r.hit_numbers)))})"
                )

        print("=" * 60 + "\n")

    def run_retroactive_backtest(
        self,
        pool_size: int = 12,
        guarantee: int = 4,
        ...
        n_draws = len(self.draws)
        eligible_indices = simulation_indices(
            self.df.attrs["training_cutoffs"], backtest_depth_percent, simulation_step
        )
        n_simulate = len(eligible_indices)

        logger.info(
            f"[BACKTEST RETROACTIV] Simulăm pentru {n_simulate} extrageri din {n_draws}"
        )
        ...
        try:
            ...
            _has_adaptive = False
            logger.warning(
                "[BACKTEST] adaptive_feedback indisponibil — fără telemetrie de regim în acest backtest."
            )
        ...
        if skip_indices:
            ...
            logger.info(
                "[BACKTEST RETROACTIV] Sar %d pași deja validați (cache parțial); rămân %d.",
                _before - len(sim_indices),
                len(sim_indices),
            )
            if not sim_indices:
        ...
        if _stateless and len(sim_indices) > 1:
            ...
            try:
                ...
                ms_per_step = (
                    (time.perf_counter() - t_probe) / max(1, probe_done) * 1000.0
                )
                ...
                if go_serial:
                    ...
                    for a in remaining:
                        ...
                        if pct >= last_log_pct + 10:
                            ...
                            logger.info(
                                "[BACKTEST] WF progres: %d/%d pași (%d%%)",
                                done_count,
                                n_steps,
                                pct,
                            )
                            ...
                            while pending:
                                ...
                                for fut in done_set:
                                    try:
                                        ...
                                        logger.error(
                                            "[BACKTEST] WF future eșuat sim_idx=%s: %s",
                                            futures.get(fut),
                                            exc,
                                        )
                                    done_count += 1
                                    _wf_report_progress(
                                        progress_cb, done_count, n_steps
                                    )
                                    pct = int(done_count * 100 / max(1, n_steps))
                                    if pct >= last_log_pct + 10:
                                        last_log_pct = pct - (pct % 10)
                ...
                logger.warning(
                    ...
                    "păstrez cele %d predicții deja calculate.",
                    exc,
                    len(retro_predictions),
                )
                cancelled = False
                ...
                if retro_predictions:
                    ...
                    logger.info(
                        ...
                        "Parțial" if cancelled else "Complet",
                        _mode,
                        len(retro_predictions),
                        avg_hits,
                    )
        ...
        for sim_num, sim_idx in enumerate(sim_indices, 1):
            ...
            if should_cancel is not None and sim_num > 1:
                try:
                    if should_cancel():
                        logger.warning(
                            "[BACKTEST] oprire timpurie la %d/%d (anulare/buget de timp) "
                            "→ validare parțială.",
                            sim_num - 1,
                            n_simulate,
                        )
            ...
            try:
                ...
                if retro_pred is None:
                    logger.warning(
                        f"[BACKTEST] Prea puține date istorice la simulare {sim_num}"
                    )
                    continue
                ...
                logger.info(
                    f"[BACKTEST RETROACTIV] Rezultat: {retro_pred.hits} numere ghicite "
                    f"(Max dintr-o varianta), pool_hits={retro_pred.hits_union}"
                )
                ...
                if use_feedback and _has_adaptive:
                    ...
                    event, info = compute_post_draw_feedback(
                        last_pool=list(retro_pred.hard_core or []),
                        actual_draw=list(retro_pred.actual_numbers),
        ...
        if retro_predictions:
            ...
            avg_hits = total_hits / len(retro_predictions)
            logger.info(
                f"[BACKTEST RETROACTIV] Complet: {len(retro_predictions)} simulări, medie {avg_hits:.2f} hits"
            )
