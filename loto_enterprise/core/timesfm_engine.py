...
def get_timesfm_scores_v2(
    ...
    try:
        for current_ctx in context_windows:
            ...
            # Heartbeat per window — UI poll-uiește astea ca să nu pară blocat
            # timp de 1-3s între ferestre.
...
def get_regressive_blacklist_v2(
    ...
    step_progress_cb: callable(done, total, step_pct) opțional — apelat după
    fiecare pas regresiv finalizat (pentru UI progress bar).
    """
    if not HAS_TIMESFM:
        return set()
    ...
    for step in steps:
        ...
        # dacă scorerul principal e bench-winner.
