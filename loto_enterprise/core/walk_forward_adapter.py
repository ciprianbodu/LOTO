...
#      `hits` (max pe un bilet) e neschimbat. WF nu persistă coverage_pct:
#      la acoperire 100% și g≥3, 3+ pool ⇔ 3+ bilet; sub 100% hits_union
#      supra-numără vs bilete; 5+/6 rămân plafon (g intern = max 4).
...
def _wheel_sig(pool_size: int, game_type: str | None = None) -> str:
    ...
    raport. Fără wheel în cheie,     schimbarea ILP → La Jolla (doar 6/49 pool 12 / g4: ~54 → 41
    bilete; covering_designs/ are doar C_12_6_4 și C_12_5_4) a servit în continuare cache vechi, iar raportul arăta costuri umflate cu
