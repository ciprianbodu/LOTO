...
def _pipeline_cache_key(input_hash: str) -> str:
    cache_key = f"{PIPELINE_CACHE_VERSION}:{input_hash}" if input_hash else ""
    # New opt-in method must never reuse a result generated by the default.
    if (
        cache_key
        and os.environ.get("LOTO_WHEEL_METHOD", "").strip().lower() == "maxcover"
    ):
        cache_key += ":wheel=maxcover-v1"
    return cache_key
...
def _run_pipeline_job_inner(job: dict, monitor: ResourceMonitor) -> str | None:
    ...
    cache_key = _pipeline_cache_key(input_hash)
